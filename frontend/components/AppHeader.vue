  ...
  <header class="fixed z-10 top-0 w-full">
    <nav class="flex justify-evenly items-center h-16" sizes="64px">
      ...
      <div class="border rounded-full p-2 bg-white">
        <NuxtLink v-for="(item, index) in navlinks" :key="index" :to="item.href" class="px-3 py-1 text-center" :class="{ 'rounded-full bg-emerald-400': $route.fullPath === item.href, 'text-black':$route.fullPath !== item.href }">{{ item.name }}</NuxtLink>
      </div>
      
      <div class="flex gap-2 items-center">
        <ClientOnly>
          <Button :icon="{'pi pi-sun': !isDarkTheme, 'pi pi-moon': isDarkTheme}" severity="secondary"  @click="toggleColorScheme()" class="!border-2 !border-gray-500" size="small" />
        </ClientOnly>
